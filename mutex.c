...
void* Thread_work(void* rank) {
    ...
    while (total_count < my_total_ops) {
	
        // Random variable to generate values for operations
        int random_value = my_rand(&seed) % MAX_RANDOM;

        // Random variable to select the operation
        ...
        if (which_op == 0 && finished_member == 0) {
            ...
            if (count_member < my_member_count && finished_member == 0) {
        ...
        // Insert Operation
            ...
            if (count_insert < my_insert_count && finished_insert == 0) {
		pthread_mutex_lock(&mutex);
        ...
        // Delete Operation
            ...
            if (count_delete < my_delete_count && delete_finished == 0) {
		pthread_mutex_lock(&mutex);
        ...
        // Updating the count
        total_count = count_insert + count_member + count_delete;
    }
    return NULL;
}
